...
export const GrammarTable = ({ data }: { data: GrammarTableProps }) => {
  return (
    <div className="overflow-hidden rounded-3xl border border-white/10 bg-white/5 backdrop-blur-md">
      {data.caption && (
        <div className="border-be border-white/10 bg-white/5 px-6 pbs-4 pbe-4">
          ...
          <thead className="bg-white/5 text-xs text-white/60 uppercase">
            <tr>
              {data.headers.map((header) => (
                <th
                  key={header}
                  className="px-6 pbs-4 pbe-4 font-semibold tracking-widest text-orange/80"
                >
                  {header}
                </th>
              ))}
          ...
          <tbody className="divide-y divide-white/5">
            {data.rows.map((row) => {
              const rowKey = row
                ...
                .join("|");
              return (
                <tr key={rowKey} className="transition-colors hover:bg-white/5">
                  {row.map((cell, j) => {
                    const cellText =
                    ...
                    return (
                      <td
                        key={cellText}
                        className={`px-6 pbs-4 pbe-4 ${j === 0 ? "font-medium text-orange" : "text-white/90"}`}
                      >
